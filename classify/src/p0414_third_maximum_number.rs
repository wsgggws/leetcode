...
impl Solution {
    pub fn third_max(nums: Vec<i32>) -> i32 {
        ...
        for i in 0..nums.len() {
            if nums[i] == i32::min_value() {
                is_exist_min_value = true;
            }
            ...
            if nums[i] > max_first {
                ...
                max_third = max_second;
                max_second = nums[i];
            } else if nums[i] > max_third && nums[i] != max_second && nums[i] != max_first {
                max_third = nums[i];
            }
        }

        // 存在 max_third 的情况, 第三大值确实是 i32::min_value()
        if is_exist_min_value && (max_second != i32::min_value()) {
            return max_third;
        }
        // 存在 max_third 的情况, 第三大值> i32::min_value()
        else if max_third > i32::min_value() {
            return max_third;
        }
        // 不存在 max_third 的情况
        else {
...
mod tests {
    ...
    fn third_max_test() {
        ...
        assert_eq!(
            Solution::third_max(vec![3, 2, 2, i32::min_value()]),
            i32::min_value()
        );
        assert_eq!(
            Solution::third_max(vec![i32::min_value(), 2, 2, i32::min_value()]),
            2
        );
    }
}
