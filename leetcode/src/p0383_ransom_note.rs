...
impl Solution {
    pub fn can_construct(ransom_note: String, magazine: String) -> bool {
        ...
        while index_j < magazine.len() && index_i < ransom_note.len() {
            if magazine[index_j] == ransom_note[index_i] { index_i += 1; }
            index_j += 1_usize;
        }
        if index_i == ransom_note.len() { true } else { false }
    }
}
...
mod tests {
    ...
    fn can_construct_test() {
        assert_eq!(Solution::can_construct("a".to_owned(), "b".to_owned()), false);
        ...
        assert_eq!(Solution::can_construct("aba".to_owned(), "aabaa".to_owned()), true);
        assert_eq!(Solution::can_construct("bjaajgea".to_owned(), "affhiiicabhbdchbidghccijjbfjfhjeddgggbajhidhjchiedhdibgeaecffbbbefiabjdhggihccec".to_owned()), true);
    }
}
