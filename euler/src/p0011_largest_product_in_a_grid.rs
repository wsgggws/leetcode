...
impl Solution {
    pub fn largest_product_in_a_grid(n: usize) -> u64 {
        let grid = vec![
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                if i + 3 < n && j + 3 < n {
                    groups.push((grid[i][j], grid[i+1][j+1], grid[i+2][j+2], grid[i+3][j+3]));
                }
                // 不要忘记有四个方向，特别是这个方向
                if i + 3 < n && j >= 3 {
                    groups.push((grid[i][j], grid[i+1][j-1], grid[i+2][j-2], grid[i+3][j-3]));
...
mod tests {
    ...
    #[test]
    fn largest_product_in_a_grid_test() {
        assert_eq!(Solution::largest_product_in_a_grid(4), 24468444);
        assert_eq!(Solution::largest_product_in_a_grid(5), 24468444);
        assert_eq!(Solution::largest_product_in_a_grid(6), 32719995);
        assert_eq!(Solution::largest_product_in_a_grid(20), 70600674);
    }
}
